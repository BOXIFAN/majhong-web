...
TRANSLATIONS = {
    "zh": {
        ...
        "experiment.experimental_note": "实验功能：结果仅作参考。已纳入 M-League 常规赛通算的 54 位职业选手官方个人成绩进行拟合；受公开数据缺少逐局/逐顺位分数明细所限，部分指标为近似推算。其中「平均打点」「火力」采用 Mリーグ积分单位（场均 / 每次一位均值），口径与本站原始分数不同，不代表官方统计或对牌风的权威评价。",
